...
// *********************** Single Image Upload with aspect ratio *****************
...
// import { dateFormatter } from "./dateTimeFormatters";
// import CameraIcon from '@/assets/camera.png'
// import DefaultImageBgCircle from '../../../public/assets/templates/defultImageBgCircle.png'
// import DefaultImageBgRectangle from '../../../public/assets/templates/defultImageBgRectangle.png'
// import Cropper from 'react-easy-crop';
// import { FaCropAlt } from "react-icons/fa";
// import { BiZoomOut } from "react-icons/bi";
// import { BiZoomIn } from "react-icons/bi";
...
//     address: formData?.address,
//     templateId: templateId || "",
//     image: uploadedImage ? uploadedImage : cropShape === 'round' ? DefaultImageBgCircle.src : DefaultImageBgRectangle.src,
//   });
//   console.log('%c [ dataForTemplate ]-567', 'font-size:13px; background:pink; color:#bf2c9f;', dataForTemplate)
//   console.log('%c [ dateFormatter(formData?.date, template?.dateFormatCase)?.day ]-571', 'font-size:13px; background:pink; color:#bf2c9f;', dateFormatter(formData?.date, template?.dateFormatCase)?.day)
...
//     return () => {
//       imgRef.current?.removeEventListener("load", updateHeight);
...
//             <div  style={{ position: "absolute", zIndex: 2, top: 0, left: 0, right: 0, bottom: 0, height: imgHeight }}>
//               <div class="invite-template-wrapper" style={{height: imgHeight}}>
//                 <div class="invite-template-card" style={{height:  imgHeight }}>
...
// ************************* Single Image Upload without aspect ratio
"use client";
...
const DynamicTemplateRenderer = () => {
        ...
        if (templateId) {
          ...
          if (result.error) {
            ...
            if (selectedTemplate) {
              ...
              const heroCropShape =
                selectedTemplate?.configs?.heroImageConfig?.cropShape || "rect";
              setCropShape(heroCropShape);
              let heroAspect =
                (parseInt(
                  selectedTemplate?.configs?.heroImageConfig?.cropRatio?.width
                ) || 4) /
                (parseInt(
                  selectedTemplate?.configs?.heroImageConfig?.cropRatio?.height
                ) || 3);
              if (heroCropShape === "round") {
                heroAspect = 1;
              }
              ...
              setCropSize({
                width:
                  parseInt(
                    selectedTemplate?.configs?.heroImageConfig?.cropSize?.width
                  ) || 200,
                height:
                  parseInt(
                    selectedTemplate?.configs?.heroImageConfig?.cropSize?.height
                  ) || 200,
              });
              ...
              setTemplate({
                ...
                charLimits: selectedTemplate.configs?.charLimits || {},
                dateFormatCase:
                  selectedTemplate?.configs?.dateFormatCase || "1",
              });
            } else {
              setError("Template not found");
            }
  ...
  const handleChange = (e) => {
    ...
    // Always allow change but trim if exceeds limit
    const newValue =
      value.length > charLimit ? value.slice(0, charLimit) : value;
      ...
      <div style={{ padding: "10px", maxWidth: "500px", width: "100%" }}>
        <div
          ...
          style={{
            ...
            backgroundPosition: "center",
            backgroundRepeat: "no-repeat",
            maxHeight: template?.bgImageHeight
              ? `${template?.bgImageHeight}px`
              : "530px",
            maxWidth: "480px",
            width: "100%",
            borderRadius: "10px",
...
{{/if}}
        ...
        <div className="form-wrapper">
          <h3 className="heading-txt">
            Do you Want <br /> customize Invite?
          </h3>
          <div style={{ position: "relative" }}>
            <input
              type="text"
          ...
          {template?.isHeroImage &&
            (originalImage ? (
              <div className="preview-wrapper-template">
                <div className="preview-image-div">
                  <img
                    src={originalImage}
                    alt="Preview"
                ...
                <div className="d-flex justify-content-center align-items-end ms-2">
                  <button className="crop-btn" onClick={handleEditImage}>
                    <FaCropAlt className="crop-icon" /> Crop
                  </button>
                </div>
              </div>
            ) : (
              <label className="upload-box-template" htmlFor="file-upload">
                <img
                  ...
                  alt="Upload"
                />
                <div className="mt-2 fw-bold" style={{ color: "#666666" }}>
                  Upload Photo
                </div>
              </label>
            ))}
