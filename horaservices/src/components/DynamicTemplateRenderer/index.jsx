...
useEffect(() => {
 ...
 <div
      className="template-image-wrapper1"
      style={{
        marginTop: '50px',
        width: '150px',
        height: '150px',
        overflow: 'hidden',
        position: 'relative',
        backgroundColor: 'grey', // for debug
        border: '2px solid red',
        borderRadius: '50%',
        touchAction: 'none', // prevent gesture conflicts
        overflow: 'hidden',
      }}
    >
      <PanZoom
        ...
        maxZoom={10}
        zoomSpeed={2}
         x={1} // Initial horizontal offset (optional)
        y={1}
        style={{
          width: '100%',
          height: '100%',
      ...
      >
        <img
          src="http://localhost:3000/_next/static/media/zoom-img-dummy.jpg"
          alt="host image"
          className="template-image1"
          style={{
           width: '300px', // larger than container to allow zoom/pan
            height: '300px',
            objectFit: 'contain',
            cursor: 'grab',
            userSelect: 'none',
  ...
  <div class="invite-template-card" style="height:{{imgHeight}}px;">
    ...
    <div class="name" style="font-size:{{nameFontSize}}px; top:{{namePosition}}px; position:absolute; line-height:{{nameLineHeight}}px;">
     {dataForTemplate?.time}
    </div>

    <div class="date-wrapper" style="top:{{dateTimePosition}}px; position:absolute; line-height:{{dateTimeLineHeight}}px;">
      <span class="month" style="font-size:{{dateTimeFontSize}}px;">{dataForTemplate?.time}</span>
      <span class="day" style="font-size:30px;">{dataForTemplate?.time}</span>
      <span class="time" style="font-size:{{dateTimeFontSize}}px;">{dataForTemplate?.time}PM.</span>
    </div>
