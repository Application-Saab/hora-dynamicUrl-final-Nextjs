...
// Agar ye constants alag file me hain to path adjust kar lena
import {
  BASE_URL,
  PAYMENT_STATUS,
  UPDATE_ORDER_STATUS,
...
export default function Home({
  ...
  // Client-side navigation pe city update
  useEffect(() => {
    if (router.isReady) {
  ...
  // Payment status — client only (window / localStorage)
  useEffect(() => {
      ...
      try {
        ...
        if (response.data && response.data.message) {
          ...
          if (message === "PAYMENT_SUCCESS") {
            ...
            await axiosApi.post(url, requestData, {
              headers: {
                "Content-Type": "application/json",
                authorization: token,
              },
  ...
  const openSliderLink = (link, cityParam) => {
    if (link) {
      window.location.href = `/${cityParam}/${link}`;
    }
  };
                ...
                <div key={index} className="slide-container">
                  ...
                  <div className="carousel-content slide-content">
                    ...
                    <button
                      ...
                      onClick={() => openSliderLink(slide.link, displayCity)}
                    >
                      book Now
                    </button>
                  </div>
          ...
          <div className="categoriesCard-container">
            {whereAreYouData.map((category) => (
              <div key={category.id} className="categoriesCard-card">
                <a
                  href={`${displayCity}/${category.link}`}
