...
// Route: /[city]/venue-list/venue?venueid=...
const CityVenuePage = () => {
  const router = useRouter();
  const { city, venueid: queryVenueId } = router.query;
  ...
  // ── Event/Venue Details: cache-first (city-scoped cache key) ──
  useLayoutEffect(() => {
    const fetchEventDetails = async () => {
      if (!queryVenueId || !loggedinUserId) return;

      const cacheKey = `venue-details-${city || "default"}-${queryVenueId}`;
    ...
    fetchEventDetails();
  }, [city, queryVenueId, loggedinUserId]);
  ...
  // ── Venue Packages: cache-first (city-scoped cache key) ──
  useLayoutEffect(() => {
    const fetchVenuePackage = async () => {
      if (!queryVenueId || !loggedinUserId) return;

      const cacheKey = `venue-packages-${city || "default"}-${queryVenueId}`;
      const cached = getPageCache(cacheKey);
  ...
  useLayoutEffect(() => {
    if (eventData?.data) {
      ...
      setFullPageLoader(false);
      if (queryVenueId) {
        setPageCache(`venue-details-${city || "default"}-${queryVenueId}`, eventData.data);
      }
    }
  }, [eventData]);
    ...
    <>
      ...
      <CreateInviteModal
        ...
        getRedirectRoute={(data) => ({
          pathname: `/${city || "default"}/venue-list/venue`,
