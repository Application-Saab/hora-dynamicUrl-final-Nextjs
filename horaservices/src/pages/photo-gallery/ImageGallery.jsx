  ...
  useEffect(() => {
    const fetchThumbnails = async () => {
      ...
      try {
        ...
        const fetchedThumbnails = (data.thumbnails || []).map((thumb, index) => ({ ...thumb, stableKey: thumb.id || thumb.uniqueKey || thumb.thumbnailImageUrl || `thumb-gallery-${index}-${Date.now()}` }));
        ...
        <div className="masonryGrid">
          {currentThumbnailsOnPage.map((thumbnail, indexOnPage) => (
            <LazyImage
              ...
              onClick={() => inlarge && handleImageClick(indexOnPage)}
            />
          ))}
        </div>
      ) : (
            ...
            >
              {allThumbnails.map((thumb, idx) => (
                <div key={thumb.stableKey || `slide-gallery-${idx}`} className="slick-slide-item">
                  <img
                    src={thumb.originalUrl || thumb.thumbnailImageUrl}
