    ...
    if (apiProduct && !isFetched) {
      const fetchDecorationDetails = async () => {
        try {
          ...
          const productPrice = parseInt(fetchedProduct.price, 10);

          // Extract first two words
          const words = fetchedProduct.name.split(" ");
          const firstTwoWords = words.slice(0, 2).join(" ").toLowerCase();

          // Check for "ring" or "light"
          const containsRing = /\bring\b/i.test(fetchedProduct.name);
          const containsLight = /\blight\b/i.test(fetchedProduct.name);

          // Fetch all products
          const tagResponse = await axios.get(
            `https://horaservices.com:3000/api/Decoration/searchByTag/${fetchedProduct.tag[0]}`
          );
          const allProducts = tagResponse.data.data || [];
          ...
          // Separate searches for "ring" and "light"
          if (containsRing) {
            filteredProducts = fuse.search("ring").map((res) => res.item);
          } else if (containsLight) {
            filteredProducts = fuse.search("light").map((res) => res.item);
          } else {
            filteredProducts = fuse
              .search(firstTwoWords)
              .map((res) => res.item);
          }
          ...
          let finalProducts = filteredProducts.filter((p) => {
            ...
            if (productPrice < 2000) {
              return price < 2000;
            } else if (productPrice >= 2000 && productPrice <= 4000) {
          ...
          setSimilarProducts(finalProducts.slice(0, 10));
