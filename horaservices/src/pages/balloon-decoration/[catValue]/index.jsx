...
const DecorationCatPage = () => {
  ...
  }, [catalogueData, priceFilter, themeFilter]);
  ...
  const getSubCatItems = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `${BASE_URL}${GET_DECORATION_CAT_ITEM}${catId}?page=${page}&limit=${limit}`
      );
      ...
      if (response.status === 200) {
        ...
        setCatalogueData((prev) => [...prev, ...newItems]);
