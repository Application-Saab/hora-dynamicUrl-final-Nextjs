...
//   const SimpleTextareaWithEmoji = ({ onSend, userData, selectedGroup }) => {
...
//   // Mark read + fetch messages on mount
//   useEffect(() => {
//     if (selectedGroup && userId) {
//       const gid = selectedGroup._id || selectedGroup.id;
//       markRoomRead(gid, userId);
...
//   // ===== INPUT HANDLING: Lists, Emoji, Resize, Paste =====
//   useEffect(() => {
//     const el = textareaRef.current;
//     if (!el) return;
...
//       return !!findParent(sel.anchorNode, "LI");
//     };

//     const isAtListStart = () => {
//       const sel = window.getSelection();
//       if (!sel || !sel.anchorNode) return false;
//       return sel.anchorOffset === 0 && isInList();
//     };
...
//       if ((lastLine.startsWith("1. ") || lastLine.startsWith("- ")) && lastLine.length > 3) {
//         const isNumbered = lastLine.startsWith("1. ");
//         const listType = isNumbered ? "ol" : "ul";
//         const content = lastLine.slice(3).trim();

//         if (lines.length === 1) {
//           el.innerHTML = `<${listType}><li>${content}</li></${listType}>`;
//           const range = document.createRange();
...
//         e.preventDefault();
//         insertNewListItem();
//       } else if (e.key === "Backspace" && isAtListStart()) {
//         e.preventDefault();
//         exitList();
//       }
//     };
...
//     // Expose insertEmoji to EmojiPickerButton if needed
//     el.dataset.insertEmoji = insertEmoji;

//     return () => {
//       el.removeEventListener("input", handleInput);
//       el.removeEventListener("keydown", handleKeyDown);
//       el.removeEventListener("paste", handlePaste);
//       el.removeEventListener("keyup", saveCursor);
//       el.removeEventListener("mouseup", saveCursor);
//       el.removeEventListener("focus", saveCursor);
//     };
//   }, []);

//   // Scroll to unread/bottom
//   useLayoutEffect(() => {
//     if (!messages.length || !selectedGroup || !chatBodyRef.current) return;
//     if (hasScrolledToUnreadRef.current) return;
...
//         if (firstUnreadIndex !== -1) {
//           const messageElements = chatBodyRef.current.querySelectorAll(".chat-message");
//           const target = messageElements[firstUnreadIndex];
//           if (target) {
...
//         socket.emit("message:read", { groupId, userId });
//       }
//       const resp = await markReadRequest(`${MARK_READ_MESSAGE}`, "POST", { groupId, userId });
//       if (!resp.error && (resp.unreadCounts || resp.data?.unreadCounts)) {
//         setUnreadCountsContext((prev) => ({
//           ...prev,
//           ...(resp.unreadCounts || resp.data.unreadCounts),
//         }));
...
//       try {
//         const resp = await fetchUserRequest(`${GET_USER_BY_ID}/${userId}`, "GET");
//         if (resp?.data) setUserData(resp.data);
//       } catch (err) {
...
//   const handleBack = () => {
//     const basePath = "/chat";
//     router.push(userId ? `${basePath}?id=${encodeURIComponent(userId)}` : basePath);
//   };
...
//         <div className="chat-header">
//           <div className="chat-user-info">
//             <Image src={backIcon} alt="Back" className="back-arrow-img" onClick={handleBack} />
//             {roomDisplayDetails?.avatar ? (
//               <img src={roomDisplayDetails.avatar} alt={roomDisplayDetails.name} className="chat-group-img" />
//             ) : (
//               <div className="placeholder-avatar">{roomDisplayDetails?.avatarText}</div>
//             )}
...
//               {!isMe && !isConsecutive && (
//                 membersProfileMap[msg.senderId] ? (
//                   <img src={membersProfileMap[msg.senderId]} alt="avatar" className="chat-avatar-receiver" />
//                 ) : (
//                   <div
//                     className="chat-avatar-receiver"
//                     style={{ backgroundColor: getAvatarColor(msg.senderName || msg.senderPhone) }}
//                   >
...
//                 {!isMe && !isConsecutive && (
//                   <div className="chat-sender">{msg.senderName || "+91 XXXX"}</div>
//                 )}
//                 <div className="chat-text" dangerouslySetInnerHTML={{ __html: msg.html || msg.message || "" }} />
//               </div>
//             </div>
//           ) : (
//             <div key={msg._id} className="d-flex justify-content-center align-items-center" style={{ margin: "12px 0" }}>
//               <p className="info-chat-message-box">{msg?.message}</p>
//             </div>
//           );
//         })}
...
//         <EmojiPickerButton
//           onEmojiSelect={(emoji) => {
//             const insertFunc = textareaRef.current?.dataset?.insertEmoji;
//             if (insertFunc && typeof window[insertFunc] === "function") {
//               // Alternative: directly call the function if exposed
//             } else {
//               // Simple fallback: just insert as text
