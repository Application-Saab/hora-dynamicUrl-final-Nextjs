...
import './Eventinvitation.css'; // CSS same as before

const InviteCard = () => {
  // Dummy data to mimic actual API response
    const router = useRouter();
  const inviteData = {
    hostName: 'Simran',
    eventDate: '2025-07-18T00:00:00.000Z',
    eventTime: '06:00 AM',
    location: 'Home (12 A, Comfort Lushgreen)',
  };

  const formattedDate = new Date(inviteData.eventDate).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric'
  });
  ...
  return (
    <div className="invite-card">
      <p className="invite-heading">YOU ‘RE INVITED</p>

      <Image  alt="Cake" width={80} height={80} />

      <h2 className="event-title">{inviteData.hostName.toUpperCase()} ‘S BIRTHDAY CELEBRATION</h2>

      <p className="event-row">📅 <strong>DATE :</strong> {formattedDate}</p>
      <p className="event-row">⏰ <strong>TIME :</strong> {inviteData.eventTime}</p>
      <p className="event-row">📍 {inviteData.location}</p>
